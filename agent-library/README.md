...
- `agent-library/agents/polymarket-staked-external-settlement/`: deterministic external-settlement example that ingests signed agent trade/settlement commands, publishes cumulative trade-log snapshots through the message publication node, tracks node reimbursement classifications, disputes user withdrawals while markets remain unsettled, and proposes reimbursement only after the required settlement deposit is complete.
