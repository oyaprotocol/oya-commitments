...
print_usage() {
  cat <<'EOF'
Usage:
  bash script/add-safe-owners.sh --safe <safe-address> --owners <csv> [forge script args...]

Example:
  ENV_FILE=agent/.env bash script/add-safe-owners.sh --safe 0xSafe --owners 0x1111...,0x2222...

Notes:
  - This script adds the supplied owners and finalizes the Safe as an N/N owner set.
  - It is intended for the phase where the deployer still controls the Safe with threshold 1.
EOF
}
...
export SAFE_OWNER_ACTION="add"
export SAFE_ADD_OWNERS="$OWNERS_VALUE"
