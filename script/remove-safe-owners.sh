...
print_usage() {
  cat <<'EOF'
Usage:
  bash script/remove-safe-owners.sh --safe <safe-address> --owners <csv> [forge script args...]

Examples:
  ENV_FILE=agent/.env bash script/remove-safe-owners.sh --safe 0xSafe --owners 0x1111...
  ENV_FILE=agent/.env bash script/remove-safe-owners.sh --safe 0xSafe --owners 0x1111...,0x2222...

Notes:
  - If the supplied removals would leave no owners, the Safe is rotated to the dead address.
  - This script is intended for the phase where the deployer still controls the Safe with threshold 1.
EOF
}
...
[[ -n "$OWNERS_VALUE" ]] || { echo "--owners is required" >&2; exit 1; }

export COMMITMENT_SAFE="$SAFE_ADDRESS"
export SAFE_OWNER_ACTION="remove"
export SAFE_REMOVE_OWNERS="$OWNERS_VALUE"
