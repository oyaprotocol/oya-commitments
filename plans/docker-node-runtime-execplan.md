...
- [x] 2026-09-21: Milestone 4 complete: added configurable general message-request limits and operator documentation. All five focused tests, all 94 host tests, and the internal Docker flow on Linux arm64 passed. Ready for the requested human review.
...
- External ingestion can reuse `POST /v1/messages` and `scripts/send-message.mjs`; the present deployment publishes the API only on host loopback. Existing body/time/connection limits and one-operation admission bound individual work; milestone 4 adds the general request-rate limit. Repeated signed envelopes already produce independent successful results, as checked by `scripts/smoke-local.mjs`; the user explicitly confirmed this behavior is intended.
- Milestone 4 tests can replace `performance.now()` through Node's test mock API, avoiding production clock-injection plumbing and minute-long waits. A headers-only HTTP request also verified that the limit returns HTTP 429 and closes the socket before the body arrives.
...
- Decision: Anchor fixed request windows to HTTP server creation using `performance.now()`, with the counter checked synchronously before reading a body. Rationale: A monotonic clock avoids wall-clock changes, idle windows retain their boundaries, and concurrent requests share the budget without extra timers or dependencies. Date/Author: 2026-09-21 / Codex.
...
Milestone 4 adds `maxMessageRequestsPerMinute` (default 60) and a fixed-window counter before body reading and authorization. Exhausted budgets return HTTP 429, `started: false`, a rounded-up `Retry-After`, and a closed connection. The runtime change adds 19 lines across the existing config/server files; a 139-line test file verifies configuration, shared counting, exact resets, independent repeated submissions, early connection closure, busy behavior, and restart. All 94 host tests and the existing Docker flow passed. The README explains boundary bursts and in-memory resets.
...
Milestone 4 evidence (2026-09-21):
