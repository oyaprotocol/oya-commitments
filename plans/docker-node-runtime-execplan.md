...
This ExecPlan is a living document maintained according to `PLANS.md`. The user authorized milestones 1 and 2 on 2026-09-14, milestone 3 on 2026-09-15, and milestones 4–6 on 2026-09-21, with small diffs for manual line-by-line review. Work proceeds in the milestones below, with a review handoff after each milestone.
...
- [ ] Milestone 3 hosted validation: confirm the container CI job on its native amd64 Ubuntu runner. Hosted CI status, native Linux host permissions, and Windows file sharing remain unverified here; the user requested proceeding without checking GitHub.
...
- [x] 2026-09-21: Milestone 4 initial implementation: added configurable general message-request limits and operator documentation. All five focused tests, all 94 host tests, and the internal Docker flow on Linux arm64 passed. The user subsequently requested counting only valid allowlisted messages.
- [x] 2026-09-21: Moved the message-limit check after signature and allowlist validation as requested. Malformed, forged, and non-allowlisted messages do not consume the budget. Updated tests and documentation; all 94 host tests and the internal Docker flow on Linux arm64 passed. Ready for human review.
- [x] 2026-09-21: Milestone 5 complete: added the optional HTTPS proxy and operator instructions using pinned Caddy 2.11.4. Merged Compose and Caddy configuration checks passed at default and longer deadlines. A local TLS fixture verified routing, preserved messages/statuses/headers, log filtering, no retry on upstream failure, and graceful Compose shutdown. Ready for manual review; full signed-publication HTTPS tests and live reachability remain milestone 6.
- [x] 2026-09-21: Added milestone 6's `--http` harness mode, nine-line HTTPS fixture overlay, npm command, CI step, and external-machine instructions.
- [x] 2026-09-21: Milestone 6 local validation passed: all 94 host tests, the original internal Docker flow, and the complete HTTPS flow on native Linux arm64. A transaction held pending for 245 seconds during shutdown returned a verified successful HTTPS response; proxy and node both exited 0. Hosted CI and the HTTPS flow on amd64 remain unverified here.
- [ ] Milestone 6 live reachability: run the documented check from another machine once an operator-owned deployment, hostname, and authorized client are supplied. Public DNS and certificate issuance remain unverified here.

## Surprises & Discoveries

- The HTTPS fixture can reuse the existing sender by supplying its generated public CA through `NODE_EXTRA_CA_CERTS` only to that child process; direct test requests use an explicit `ca` with certificate verification enabled. A client without the CA is required to fail. Container stop grace is exposed as `Config.StopTimeout` in seconds, so the test compares the running containers' values with the adapted Caddy deadlines and node configuration.
- Caddy's upstream HTTP transport can retry requests on reused connections independently of its load-balancing retry setting. The proxy disables upstream keep-alive as well as setting zero retries. Its runtime error logs can contain request metadata even without access logging, so the default logger removes the request object. A dropped upstream response verified one attempt and no fixture credentials or body content in proxy logs.
...
- External ingestion can reuse `POST /v1/messages` and `scripts/send-message.mjs`; the present deployment publishes the API only on host loopback. Existing body/time/connection limits and one-operation admission bound individual work; milestone 4 adds the shared allowlisted-message limit. Repeated signed envelopes already produce independent successful results, as checked by `scripts/smoke-local.mjs`; the user explicitly confirmed this behavior is intended.
- Milestone 4 tests can replace `performance.now()` through Node's test mock API, avoiding production clock-injection plumbing and minute-long waits. The existing `onAcceptedMessage` callback runs after signature and allowlist validation, so moving the counter there excludes unauthenticated traffic without duplicating authorization. The limiter must now read and authenticate the message before returning HTTP 429.
...
- Decision: Add an HTTPS branch to the existing Docker harness, reusing generated accounts, deployment, image build, sender, IPFS retrieval, Ledger verification, and cleanup. Retain `test:docker` for persistence/recovery/backup checks, and add `test:http` to the same CI job. Rationale: This keeps new test code focused on the public interface without duplicating the fixture or expanding production runtime code. Date/Author: 2026-09-21 / Codex.
- Decision: Validate the production port model separately from the running fixture's dynamic loopback bindings, retaining the existing test-only IPFS API mapping for independent content verification. Hold a real transaction pending for 245 seconds after proxy shutdown begins with five-minute operation/receipt deadlines and six-/seven-minute proxy response/stop settings. Rationale: This directly exercises both response and shutdown behavior beyond the previous four-minute deadline while preserving private production API bindings. Date/Author: 2026-09-21 / Codex.
- Decision: Pin official `caddy:2.11.4-alpine` by its verified Linux image index, expose TCP 80/443 with HTTP/1.1 and HTTP/2, and keep the admin API disabled. Persist `/data` and `/config` in project volumes. Rationale: This provides automatic HTTPS with a small optional overlay and no application dependency changes. Date/Author: 2026-09-21 / Codex.
- Decision: Reuse `OYA_PROXY_RESPONSE_TIMEOUT` for both upstream response-header waiting and Caddy shutdown draining; add `OYA_PROXY_STOP_GRACE_PERIOD` (default `5m`). Document operator checks for at least one minute above `operationTimeoutMs` and another minute for Docker to stop the proxy. Rationale: Compose cannot compare its durations with the private node JSON; configurable deadlines and explicit comparisons keep this milestone limited to configuration and instructions. Date/Author: 2026-09-21 / Codex.
- Decision: Count only valid, signature-verified messages from `allowedSigners`, before checking node capacity. Keep one shared counter in fixed windows anchored to HTTP server creation using `performance.now()`. Rationale: Unauthenticated senders must not consume the message budget; valid busy and repeated requests still count. The counter remains synchronous with no extra timers or dependencies. This supersedes the initial general-request policy. Date/Author: 2026-09-21 / user direction, recorded by Codex.
...
Milestone 3 implements a repository-owned Docker harness and small test overlay, reusing the existing deployment CLI and sender. Native arm64 and emulated amd64 runs verified signed publication, exact IPFS bytes and Ledger events, container recreation with persistent identity/pins/content, busy and unauthorized rejection, draining a pending transaction beyond ten seconds during SIGTERM, and cold backup restore with matching ownership/mode. A receipt timeout left the container unhealthy without restarting; its receipt was mined and verified before a deliberate restart accepted new work. All 89 host tests and a separate arm64 OCI export passed. The new CI job is configured for native amd64 execution and an arm64 cross-build; hosted results remain unverified here. No image was published and no live deployment was performed.

Milestone 4 adds `maxMessageRequestsPerMinute` (default 60) and a fixed-window counter after signature and allowlist validation. Valid allowlisted requests at an exhausted budget return HTTP 429, `started: false`, a rounded-up `Retry-After`, and a closed connection. Malformed, forged, and disallowed messages retain their validation errors without consuming the budget. Five focused tests cover configuration, a shared budget across allowlisted signers, exact resets, independent repeated submissions, authorization before limiting, busy behavior, and restart. The README explains boundary bursts and in-memory resets. All 94 host tests and the internal Docker flow passed with the revised counting policy.

Milestone 5 adds the optional `docker/compose.http.yaml` and `docker/Caddyfile`, plus operator instructions for DNS, firewall access, certificates, deadlines, external senders, and shutdown. The proxy exposes only the message path through HTTPS and keeps the base node/IPFS bindings intact. Default and longer configuration checks and a disposable local TLS routing/shutdown fixture passed. Timeout comparisons against the node JSON are documented operator checks, not automated validation. The proxy can restart independently while the signing node retains its manual recovery policy.

Milestone 6 implements the HTTPS mode in the existing Docker harness, with a small test overlay and CI step. It uses the production proxy, real signed messages, IPFS content checks, and Ledger receipts/events; default and longer deadline relationships are checked against Caddy's adapted configuration and actual container stop settings. The HTTPS flow passed in about 347 seconds on native Linux arm64, including rejection/limit recovery, independent identical submissions, and graceful shutdown with a successful client response after holding a transaction pending for 245 seconds. The original internal Docker regression and all 94 host tests also passed. The README includes a separate remote sender and port-check procedure. Live reachability, public DNS/certificate issuance, the HTTPS flow on amd64, and hosted CI results remain unverified here.
...
### Milestone 4: Shared limits on allowlisted message requests
...
Add a positive integer `maxMessageRequestsPerMinute`, defaulting to 60. Use one process-wide counter shared by all allowlisted signers in fixed 60-second windows, measured with a monotonic clock. Count only valid `POST /v1/messages` requests after body validation, signature verification, and the `allowedSigners` check, including busy and repeated requests. Malformed requests, invalid signatures, and disallowed signers must not consume this budget. Once exhausted, return HTTP 429 for valid allowlisted requests with `{ "code": "rate_limited", "started": false }`, a `Retry-After` value for the remaining window, and a closed connection. Health checks and other routes do not consume this budget. The counter resets on process restart; document that fixed windows permit bursts across a boundary. Retain existing body-size, body-timeout, connection, and single-operation limits.

Requests must pass the existing signature and `allowedSigners` checks before reaching the counter; a claimed signer address alone is insufficient. Do not interpret a repeated signature or CID as a reason to reject a request. Acceptance covers configuration validation, the exact threshold and reset boundary using a controlled clock, no publication on a limited request, continued health checks at the limit, and normal validation/authorization failures both below and at the limit without consuming capacity. Repeated valid messages below the limit must still be processed independently. Run the production host tests and the existing internal Docker flow before this milestone's review handoff.
...
Make the proxy response deadline configurable through `OYA_PROXY_RESPONSE_TIMEOUT`, defaulting to `4m`. Require its resolved duration to be at least `operationTimeoutMs + 60_000` milliseconds; document this operator comparison and the necessary overrides whenever `operationTimeoutMs` changes. Set Caddy's shutdown drain allowance to the same response deadline. Use `OYA_PROXY_STOP_GRACE_PERIOD` (default `5m`) for Docker, at least one minute longer than the proxy response deadline. Keep the node's `OYA_STOP_GRACE_PERIOD` at least one minute above its operation deadline. Order shutdown so the proxy drains while the node and Kubo remain available. Compose and Caddy syntax validation do not compare these durations with the private node JSON.
...
docker compose -p oya -f node/production/compose.yaml -f node/production/docker/compose.http.yaml run --rm --no-deps proxy caddy validate --config /etc/caddy/Caddyfile --adapter caddyfile
docker compose -p oya -f node/production/compose.yaml -f node/production/docker/compose.http.yaml up -d --build
```

Adjust those durations for the configured operation deadline before running the commands; see the README's required comparisons. For a five-minute operation, use at least six minutes for proxy response/node stop and seven minutes for proxy stop. Caddy's admin API is disabled; stop and recreate the proxy to apply changes. Stop it before the node so external requests can drain while dependencies remain available.
...
7. A configurable shared limit counts only valid, signature-verified messages from `allowedSigners`, returns HTTP 429 before publication when exhausted, resets as documented, and leaves health checks usable. Malformed, forged, and disallowed requests retain their validation errors without consuming the budget. Repeated signed messages remain eligible and count like other valid allowlisted requests.
...
- `forge build --root contracts`, all 89 production host tests, script syntax, workflow YAML parsing, and whitespace checks passed. Hosted CI results, native Linux host permissions, and Windows sharing remain unverified here. All reusable test logic and configuration live in the repository; generated credentials and service data stay in disposable fixtures.

Milestone 4 initial validation evidence (2026-09-21, before the allowlist-only revision):

- `node --test node/production/test/http-limits.test.mjs` passed all five focused tests. `npm --prefix node/production test` passed all 94 tests under Node 24.21.0 with zero failures, skips, or cancellations. Controlled clock tests verify the exact reset boundary and rounded-up retry duration, including idle windows; repeated valid envelopes retain the same CID and produce distinct transactions.
- `npm --prefix node/production run test:docker -- --verbose` rebuilt the repository image and passed in about 131 seconds on native Linux arm64 through Docker Desktop / Engine 27.4.0 and Compose 2.31.0, with Foundry 1.5.1 and Kubo 0.43.0. Publication, persistent identity/pins/content, shutdown draining, unknown-outcome reconciliation before restart, and cold backup restore all passed.
- `git diff --check` passed. HTTPS routing and proxy-path validation remain milestones 5–6.

Milestone 4 revision evidence (2026-09-21):

- `npm --prefix node/production test` passed all 94 tests under Node 24.21.0, including the five updated limiter tests. Malformed, oversized, tampered, forged-signer, and non-allowlisted requests were rejected without consuming capacity before and at the limit. Two allowlisted signers shared the same budget; repeats, busy counting, fixed-window resets, and restart behavior passed.
- `npm --prefix node/production run test:docker -- --verbose` rebuilt the image and passed the complete internal flow in about 129 seconds on Linux arm64 with Engine 27.4.0, Compose 2.31.0, Foundry 1.5.1, and Kubo 0.43.0. `git diff --check` passed.

Milestone 5 evidence (2026-09-21):

- [Official Caddy image](https://hub.docker.com/_/caddy): `caddy:2.11.4-alpine@sha256:de23def33b17fb5d1290b0f6c2add1d70780e52341896c00a4c8a2a2fe9d355e`. `docker buildx imagetools inspect caddy:2.11.4-alpine` confirmed Linux amd64 and arm64 manifests; the selected binary reported `v2.11.4`. Execution used Linux arm64 through Engine 27.4.0 and Compose 2.31.0.
- The 38-line Compose overlay and 34-line Caddyfile passed merged-model and Caddy validation. Model inspection confirmed only TCP 80/443 added, unchanged node/Kubo bindings, required hostname, a read-only repository Caddyfile, persistent certificate/config volumes, no node credentials in the proxy, bounded logs, and the proxy's healthy-node dependency. An empty hostname failed before startup.
- Caddy adaptation confirmed four-minute default and six-minute overridden response/drain deadlines, with corresponding five-/seven-minute Docker proxy stop settings. A malformed response duration failed validation. Deadline relationships with the private node JSON remain explicit operator checks.
- A disposable HTTP upstream used the repository Caddyfile and Compose overlay, with only loopback fixture ports and a local CA trusted by the test client. HTTPS preserved methods, exact bodies, HTTP 200/405/429/503, and `Retry-After`. HTTP redirected to HTTPS; `/healthz`, `/`, `/v1/messages/`, and the IPFS API path returned 404 without reaching the upstream. A dropped upstream response produced HTTP 502 after exactly one attempt, and proxy logs omitted dummy credentials, query data, request metadata, and message bodies.
- With an upstream response delayed twelve seconds, `docker compose stop` drained the response successfully and both containers exited 0. The fixture removed its containers, volumes, generated certificate data, and settings. `git diff --check` passed. Full signed-publication HTTPS tests, a request pending beyond four minutes, public certificate issuance, and live external reachability remain milestone 6.

Milestone 6 evidence (2026-09-21):

- `npm --prefix node/production run test:http -- --verbose` rebuilt the repository image and passed in about 347 seconds on native Linux arm64 through Docker Desktop / Engine 27.4.0, Compose 2.31.0, Node 24.21.0, Foundry 1.5.1, Kubo 0.43.0, and Caddy 2.11.4. Verified TLS from a separate sender produced seven successful publications with exact IPFS envelopes and independent Ledger receipts/events. Malformed/disallowed signatures, body/text limits, HTTP 429 with recovery, busy responses, redirects, private routes, and production/fixture port assertions passed without extra node transactions.
- Identical submissions returned CID `bafkreick7aifbkj35po6jdpp7toto4gszdq254l3o2cc7xjbzeuwwudnby` with distinct transactions `0x807f0bc4c9c3325e5eb30b557af969783ee0bbd198872357a2c43f73e5ce5c57` and `0xbd67b4ba40b5ed3b8214fed2fc1f97b1726eefa6c60b077fddcd9d2e57e2ffb1`. These are disposable local-chain results; agent accounts remained unfunded.
- Shutdown drained pending work for twelve seconds with default deadlines and 245 seconds with five-minute operation/receipt deadlines, a six-minute proxy response/drain, seven-minute proxy stop grace, and six-minute node stop grace. Both clients received verified success, both containers exited 0, and each pending operation produced exactly one transaction and successful result log. The longer case returned transaction `0xc155c65e4b81e7ae0afa7b15a4bd95b2754d4418ee06f805bfc13555e77367b2`. Cleanup removed the generated fixture resources.
- `npm --prefix node/production test` passed all 94 tests. `npm --prefix node/production run test:docker -- --verbose` passed the original internal flow in about 130 seconds on native Linux arm64. Script syntax, workflow YAML parsing, the CI command, and `git diff --check` passed. The final stricter production port assertions also passed independently against the production model. CI retains the internal amd64 flow and arm64 build, and adds the HTTPS amd64 flow; its hosted result remains unverified.
- Live reachability, public DNS, and public certificate issuance remain pending an operator-owned deployment, hostname, and authorized client. The README documents the separate external-machine check; no live deployment or image publication was performed.
...
Milestone 4 adds the positive-integer `maxMessageRequestsPerMinute` node-config field, defaulting to 60. Milestone 5 adds required `OYA_PUBLIC_HOSTNAME`, `OYA_PROXY_RESPONSE_TIMEOUT` (default `4m`), and `OYA_PROXY_STOP_GRACE_PERIOD` (default `5m`) for the optional HTTPS overlay, plus pinned Caddy with project-scoped certificate/configuration volumes. Milestone 6's `test:http` invokes `scripts/test-docker.mjs --http`, accepts the same verbose/platform options, and adds a fixture overlay with dynamic loopback proxy ports and `localhost` certificates. Real external access additionally needs operator-controlled DNS and host networking. The signed-message schema, allowed-signer policy, and repeated-submission semantics remain unchanged.
