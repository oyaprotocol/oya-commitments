...
const LOGGER_LOG_SELECTOR = '0x41304fac'; // log(string)
...
function encodeLoggerLogCall(cid) {
    ...
    return `${LOGGER_LOG_SELECTOR}${STRING_OFFSET}${length}${paddedContent}`;
}
/** Returns null for unrelated logs; malformed matching Logger events throw. */
function decodeLoggerLogEvent(log, loggerAddress) {
    ...
    if (signature.toLowerCase() !== LOGGER_LOG_TOPIC) {
        return null;
    }
    if (log.topics.length !== 2) {
        throw new Error('Logger Log event must have exactly two topics.');
    }
    const nodeTopic = parseBytes(log.topics[1], 'log.topics[1]', 32);
    if (nodeTopic.slice(2, 26) !== '0'.repeat(24)) {
        throw new Error('Logger Log event node address must have zero padding.');
    }
    ...
    if (data.length < 128 || data.slice(0, 64) !== STRING_OFFSET) {
        throw new Error('Logger Log event data must contain offset 32 and a string length word.');
    }
    const length = BigInt(`0x${data.slice(64, 128)}`);
    const paddedLength = ((length + 31n) / 32n) * 32n;
    if (BigInt(data.length / 2) !== 64n + paddedLength) {
        throw new Error('Logger Log event data size must match its padded string length.');
    }
    ...
    if (!/^0*$/.test(data.slice(contentEnd))) {
        throw new Error('Logger Log event string must have zero padding.');
    }
    ...
    try {
        ...
        throw new Error('Logger Log event cid must contain valid UTF-8.', { cause: error });
    }
    return {
