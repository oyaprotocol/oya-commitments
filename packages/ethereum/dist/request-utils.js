import { hasRetryableNetworkErrorCode, HttpStatusError, isPlainObject, readErrorStringChain, runWithRetry, } from '@oyaprotocol/utils';
...
function buildJsonRpcBody({ id, method, params, }) {
    try {
        ...
        throw new Error('Ethereum JSON-RPC params must be JSON-serializable.', { cause: error });
    }
}
...
async function requestEthereumJsonRpcWithCustomRetryPolicy({ config, fetch, method, params = [], id, signal, }, shouldRetryJsonRpcMethod) {
    ...
    return await runWithRetry({
        ...
        signal,
        abortErrorMessage,
        shouldRetry: (error) => shouldRetryJsonRpcMethod(normalizedMethod) && shouldRetryError(error),
