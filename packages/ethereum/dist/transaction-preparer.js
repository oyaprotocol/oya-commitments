    ...
    return async ({ to, data, value, signal }) => {
        ...
        return await runWithRetry({
            ...
            run: async ({ signal: operationSignal }) => {
                ...
                const nonce = parseTransactionQuantity(await rpc('eth_getTransactionCount', [signerAddress, 'pending']), 'eth_getTransactionCount result');
                ...
                const maxPriorityFeePerGas = parseTransactionQuantity(await rpc('eth_maxPriorityFeePerGas'), 'eth_maxPriorityFeePerGas result');
                const maxFeePerGas = assertUint256(baseFee * BigInt(baseFeeMultiplier) + maxPriorityFeePerGas, 'maxFeePerGas');
...
//# sourceMappingURL=transaction-preparer.js.map