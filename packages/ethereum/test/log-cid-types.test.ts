...
const payableRequest: TransactionRequest = {
    to: options.ledgerContract, data: '0x', value: 1n,
};
...
// @ts-expect-error Ledger submission requires explicit preparation/signing.
logCid(cid, { config: options.config, fetch: options.fetch, ledgerContract: options.ledgerContract, nodeAddress: options.nodeAddress, timeoutMs: 1000, pollIntervalMs: 1 });
// @ts-expect-error No implicit expected node is inferred from the message signer.
logCid(cid, { config: options.config, fetch: options.fetch, ledgerContract: options.ledgerContract, transactionPreparer: prepare, timeoutMs: 1000, pollIntervalMs: 1 });
