...
export const fixtures = JSON.parse(readFileSync(new URL('./ledger-abi.json', import.meta.url), 'utf8'));
export const sample = fixtures.cases.find(({ name }) => name === 'message');
export const { ledgerContract, node } = fixtures;
...
export function createLog(overrides = {}) {
    return {
        address: ledgerContract, topics: [...fixtures.topics, sample.cidKeccak256Hash], data: sample.data,
...
export function createReceipt(overrides = {}) {
    return {
        ...
        from: node, to: ledgerContract, contractAddress: null,
...
export function createOptions(overrides = {}) {
    return {
        ...
        ledgerContract, nodeAddress: node, timeoutMs: 1_000, pollIntervalMs: 1,
