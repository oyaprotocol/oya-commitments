...
const { loggerContract, node } = fixtures;
...
function createLog(overrides = {}) {
    return { address: loggerContract, topics: [...topics], data: sample.data, ...overrides };
}
...
test('Logger event decoding matches independent Foundry ABI fixtures', () => {
    for (const { cid, cidKeccak256Hash, data } of fixtures.cases) {
        ...
        assert.deepEqual(decodeLoggerEvent(log, loggerContract), { node, cidKeccak256Hash, cid });
    }
});
...
test('Logger decoding rejects noncanonical recorded CIDs and mismatched hashes', () => {
    ...
    for (const cid of ['', 'bafy-test', 'café', sample.cid.toUpperCase(), ` ${sample.cid}`, `${sample.cid}/file`]) {
        ...
        assert.throws(() => decodeLoggerEvent(createLog({ data }), loggerContract), /canonical CIDv1/);
    }
    const log = createLog({ topics: [...fixtures.topics, fixtures.cases[0].cidKeccak256Hash] });
    assert.throws(() => decodeLoggerEvent(log, loggerContract), /cidKeccak256Hash must match/);
});

test('Logger event filtering requires the expected emitter and event signature', () => {
    assert.equal(decodeLoggerEvent(createLog({ address: node }), loggerContract), null);
    assert.equal(decodeLoggerEvent(createLog({ topics: [] }), loggerContract), null);
    assert.equal(decodeLoggerEvent(createLog({ topics: [`0x${'00'.repeat(32)}`] }), loggerContract), null);
    // Unrelated contracts or events need not use the Logger event's data layout.
    assert.equal(decodeLoggerEvent(createLog({ address: node, data: '0x' }), loggerContract), null);
    assert.equal(decodeLoggerEvent(createLog({ topics: [`0x${'11'.repeat(32)}`], data: '0x' }), loggerContract), null);
...
test('Logger event matching ignores hex casing and preserves indexed value casing', () => {
    ...
    assert.deepEqual(decodeLoggerEvent(createLog(), upper(loggerContract)), {
...
test('Logger event decoding preserves optional removed metadata', () => {
    for (const removed of [false, true]) {
        assert.deepEqual(decodeLoggerEvent(createLog({ removed }), loggerContract), {
            node, cidKeccak256Hash: sample.cidKeccak256Hash, cid: sample.cid, removed,
        });
    }
    assert.equal('removed' in decodeLoggerEvent(createLog(), loggerContract), false);
    for (const removed of [null, 0, 'false']) {
        assert.throws(() => decodeLoggerEvent(createLog({ removed }), loggerContract), /log.removed/);
    }
});
...
test('Logger decoder validates the expected address and matching log envelope', () => {
    for (const address of [undefined, null, 12, '0x', node.slice(0, -2), ` ${node}`]) {
        assert.throws(() => decodeLoggerEvent(createLog(), address), /loggerContract/);
        assert.throws(() => decodeLoggerEvent(createLog({ address }), loggerContract), /log.address/);
    }
    for (const log of [undefined, null, [], true]) {
        assert.throws(() => decodeLoggerEvent(log, loggerContract), /log must be a plain object/);
    }
    for (const invalidTopics of [undefined, null, '0x']) {
        assert.throws(() => decodeLoggerEvent(createLog({ topics: invalidTopics }), loggerContract), /log.topics/);
    }
    for (const invalidTopics of [
        ...
        assert.throws(() => decodeLoggerEvent(createLog({ topics: invalidTopics }), loggerContract), /topics|padding/);
    }
});
...
test('Logger decoder requires a 32-byte CID Keccak-256 hash topic', () => {
    for (const cidKeccak256Hash of [
        ...
        assert.throws(() => decodeLoggerEvent(log, loggerContract), /log.topics\[2\] must be 32-byte hex data/);
    }
});
...
test('Logger decoder rejects invalid offsets, lengths, sizes, and nonzero padding', () => {
    ...
    for (const data of invalidData) {
        assert.throws(() => decodeLoggerEvent(createLog({ data }), loggerContract), /log.data|Logger event/);
    }
});
...
test('Logger decoder rejects invalid UTF-8 instead of silently replacing bytes', () => {
    ...
    for (const content of ['ff', 'c0af', 'eda080', 'e282', 'f4908080']) {
        ...
        assert.throws(() => decodeLoggerEvent(createLog({ data }), loggerContract), /valid UTF-8/);
    }
});
...
test('Logger decoder accepts receipt logs without changing their data', () => {
    ...
    assert.deepEqual(decodeLoggerEvent(log, loggerContract), {
