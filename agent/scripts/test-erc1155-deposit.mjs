...
import { makeErc1155Deposit, makeErc1155Transfer } from '../src/lib/tx.js';
...
async function run() {
    ...
    let writeTransferArgs;
    const transferWalletClient = {
        async writeContract(args) {
            writeTransferArgs = args;
            return '0xdef456';
        },
    };

    const transferTxHash = await makeErc1155Transfer({
        walletClient: transferWalletClient,
        account,
        config,
        token,
        recipient,
    ...
    assert.equal(transferTxHash, '0xdef456');
    assert.equal(writeTransferArgs.address.toLowerCase(), token.toLowerCase());
    assert.equal(writeTransferArgs.functionName, 'safeTransferFrom');
    assert.equal(writeTransferArgs.args[0].toLowerCase(), account.address.toLowerCase());
    assert.equal(writeTransferArgs.args[1].toLowerCase(), recipient.toLowerCase());
    assert.equal(writeTransferArgs.args[2], 8n);
    assert.equal(writeTransferArgs.args[3], 4n);
    assert.equal(writeTransferArgs.args[4], '0x');
    ...
    const relayedTransferTxHash = `0x${'3'.repeat(64)}`;
    const onchainTransferTxHash = `0x${'4'.repeat(64)}`;
    const relayerTransferTransactionId = 'relayer-tx-2';
    let relayerTransferSubmitBody;
    let relayerTransferSubmitHeaders;
    let transferStatusPollCount = 0;
    const oldFetchTransfer = globalThis.fetch;
    try {
        globalThis.fetch = async (url, options = {}) => {
            const asText = String(url);
            const asLower = asText.toLowerCase();
            ...
            if (asText.endsWith('/submit')) {
                relayerTransferSubmitBody = JSON.parse(options.body);
                relayerTransferSubmitHeaders = options.headers;
                return {
                    ok: true,
                    status: 200,
                    statusText: 'OK',
                    async text() {
                        return JSON.stringify({
                            transactionID: relayerTransferTransactionId,
                            hash: relayedTransferTxHash,
                            state: 'STATE_PENDING',
                        });
                    },
                };
            ...
            if (
                asText.includes('/transaction?') &&
                (asText.includes(`id=${relayerTransferTransactionId}`) ||
                    asText.includes(`transactionID=${relayerTransferTransactionId}`))
            ) {
                transferStatusPollCount += 1;
                return {
                    ok: true,
                    status: 200,
                    statusText: 'OK',
                    async text() {
                        if (transferStatusPollCount === 1) {
                            return JSON.stringify([
                                {
                                    transactionID: relayerTransferTransactionId,
                                    hash: relayedTransferTxHash,
                        ...
                        return JSON.stringify([
                            {
                                ...
                                hash: relayedTransferTxHash,
                                state: 'STATE_CONFIRMED',
                                transactionHash: onchainTransferTxHash,
                            },
                        ]);
                    },
                };
        ...
        const relayerTransferWalletClient = {
            async signMessage() {
                return `0x${'b'.repeat(128)}1b`;
            },
        };

        const relayerTransferPublicClient = {
            async getChainId() {
                return 137;
            },
            async readContract() {
        ...
        const relayerTransferHash = await makeErc1155Transfer({
            publicClient: relayerTransferPublicClient,
            walletClient: relayerTransferWalletClient,
            account,
            config: relayerConfig,
            token,
            recipient,
        ...
        assert.equal(relayerTransferHash, onchainTransferTxHash);
        assert.equal(relayerTransferSubmitBody.type, 'SAFE');
        assert.equal(relayerTransferSubmitBody.from.toLowerCase(), account.address.toLowerCase());
        assert.equal(
            relayerTransferSubmitBody.proxyWallet.toLowerCase(),
            relayerFromAddress.toLowerCase()
        );
        assert.equal(relayerTransferSubmitBody.to.toLowerCase(), token.toLowerCase());
        assert.equal(relayerTransferSubmitBody.nonce, '13');
        assert.equal(typeof relayerTransferSubmitBody.signature, 'string');
        assert.equal(typeof relayerTransferSubmitBody.metadata, 'string');
        assert.equal(relayerTransferSubmitBody.metadata.includes('make_erc1155_transfer'), true);
        assert.equal(relayerTransferSubmitHeaders.POLY_BUILDER_API_KEY, 'builder-key');
        assert.equal(relayerTransferSubmitHeaders.POLY_BUILDER_PASSPHRASE, 'builder-passphrase');
        assert.equal(typeof relayerTransferSubmitHeaders.POLY_BUILDER_SIGNATURE, 'string');
        assert.equal(typeof relayerTransferSubmitHeaders.POLY_BUILDER_TIMESTAMP, 'string');

        const decodedTransfer = decodeFunctionData({
            abi: parseAbi([
                'function safeTransferFrom(address from, address to, uint256 id, uint256 amount, bytes data)',
            ]),
            data: relayerTransferSubmitBody.data,
        });
        assert.equal(decodedTransfer.functionName, 'safeTransferFrom');
        assert.equal(decodedTransfer.args[0].toLowerCase(), relayerFromAddress.toLowerCase());
        assert.equal(decodedTransfer.args[1].toLowerCase(), recipient.toLowerCase());
        assert.equal(decodedTransfer.args[2], 8n);
        assert.equal(decodedTransfer.args[3], 4n);
        assert.equal(decodedTransfer.args[4], '0x');
    } finally {
        globalThis.fetch = oldFetchTransfer;
    }
